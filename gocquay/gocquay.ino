...
void loop()
{
  ...
  if (!check_Pid_Created) {
    pid = new PID((double*)&angle_y, &Output, &SetPoint, 17, 3, 5, 1);//PID_K
    check_Pid_Created = 1;
  }
  /*
     process speed
  */
  ...
  setSpeedByPWN(&left, abs(currSpeed));
  setSpeedByPWN(&right, abs(currSpeed));

  if (angle_y > Y_ondinh_Min && angle_y < Y_ondinh_Max )
  {
    stopWheel(&left);
    stopWheel(&right);
    pid->SetMode(0);
